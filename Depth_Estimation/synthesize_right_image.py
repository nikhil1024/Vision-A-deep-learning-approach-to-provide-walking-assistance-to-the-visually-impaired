...
class ConvNet(nn.Module):
    def __init__(self):
        ...
        # state size. 32 x 74 x 74
        self.conv2 = nn.Conv2d(in_channels=32, out_channels=64, kernel_size=5, stride=1, padding=1)
        self.pool2 = nn.MaxPool2d(2, 2)
        self.bn2 = nn.BatchNorm2d(64)
        self.deconv2 = nn.ConvTranspose2d(in_channels=64, out_channels=33, kernel_size=10, stride=4, padding=0)

        # state size. 64 x 36 x 36
        self.conv3 = nn.Conv2d(in_channels=64, out_channels=128, kernel_size=5, stride=1, padding=1)
        self.pool3 = nn.MaxPool2d(2, 2)
        self.bn3 = nn.BatchNorm2d(128)
        self.deconv3 = nn.ConvTranspose2d(in_channels=128, out_channels=33, kernel_size=6, stride=9, padding=0)

        # state size. 128 x 17 x 17
        self.conv4 = nn.Conv2d(in_channels=128, out_channels=256, kernel_size=5, stride=1, padding=1)
        self.pool4 = nn.MaxPool2d(2, 2)
        self.bn4 = nn.BatchNorm2d(256)
        self.deconv4 = nn.ConvTranspose2d(in_channels=256, out_channels=33, kernel_size=6, stride=24, padding=0)

        # state size. 256 x 7 x 7
        self.linear1 = nn.Linear(256*7*7, 4096)
        self.linear2 = nn.Linear(4096, 2700)

        self.deconv5 = nn.ConvTranspose2d(in_channels=3, out_channels=33, kernel_size=5, stride=5, padding=0)
    ...
    def forward(self, x):
        ...
        combined_conv = F.relu(self.conv5(combined_deconv))
        # print(combined_conv.size())
        combined_conv = F.relu(self.conv5(combined_conv))
        # print(combined_conv.size())
        combined_conv = combined_conv.to(device)

        product = torch.mul(combined_conv, shifted_images)
        # print("Product:", product.size())
        selection_layer_output = torch.sum(product, dim=1).view(-1, 1, 150, 150)
        # print("Selection layer output:", selection_layer_output.size())

        prediction = F.relu(self.conv6(selection_layer_output))
        prediction = F.relu(self.conv6(prediction))
...
def training():
    ...
    model_save_name = 'model_{}_{}.pt'.format(num_epochs, lr)
...
def testing():
    model_load_path = r'F:\PyCharm Projects\Audio Assistance to the Visually Impaired\Depth_Estimation\model_100_0.0003.pt'
    model.load_state_dict(torch.load(model_load_path))
    model.eval()
    left_image_path = r"D:\left.png"
    left_image = cv2.resize(cv2.imread(left_image_path, cv2.CV_8UC1), (150, 150))
    blur = cv2.GaussianBlur(left_image, (3, 3), 0)
    ...
    left_image = left_image.view(1, 150, 150)
    ...
    generated_right_image = model(left_image.view(-1, 1, 150, 150).to(device)).to(device).view(1, 150, 150)
    ...
    generated_right_image = generated_right_image.view(150, 150).cpu().detach().numpy()
    ...
    left_image = left_image.view(150, 150)
    ...
    disparity = stereo.compute(left_image, sharpened)
...
if __name__ == '__main__':
    ...
    train_x = train_x.view(-1, 1, 150, 150)
    train_y = train_y.view(-1, 1, 150, 150)
    test_x = test_x.view(-1, 1, 150, 150)
    test_y = test_y.view(-1, 1, 150, 150)
